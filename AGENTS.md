...
- Recommended next task: prepare a least-privilege Sanity client trial. Invite each named client tester to the `vc24qe42` development project with their own Sanity account; use Viewer for review-only access or Editor for create/edit/publish testing, never a shared username/password or Administrator access. Keep the stable Vercel demo in mock-content mode until a separate Sanity-backed Preview is configured with a server-only read token and a deliberately approved test content set. Separately confirm the notification arrived at the temporary recipient and rotate the exposed-in-chat Resend key.
