...
export default async function ProtectedAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="admin-shell min-h-screen bg-[#0b0c0c] text-white">
      {admin.mode === "demo" && (
      ...
      <div className="mx-auto min-h-[calc(100vh-36px)] max-w-[1800px] lg:grid lg:grid-cols-[264px_minmax(0,1fr)]">
        <aside className="border-b border-white/10 bg-[#0b0c0c] p-5 lg:sticky lg:top-0 lg:h-screen lg:overflow-y-auto lg:border-b-0 lg:border-r lg:p-6">
          <div className="flex items-center justify-between gap-4 lg:block">
            <Link href="/admin" className="block">
              <span className="text-[0.66rem] font-bold uppercase tracking-[0.2em] text-[#dff43b]">Church Govern</span>
        ...
        <main className="min-w-0 bg-[#f5f7f8] p-5 text-slate-950 sm:p-8 lg:p-10 xl:p-12">{children}</main>
      </div>
    </div>
  );
}
