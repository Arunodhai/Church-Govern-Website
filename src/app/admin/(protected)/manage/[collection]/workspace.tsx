    ...
    <>
      <div className="mt-7 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-3 sm:flex-row sm:items-center">
        <label className="relative min-w-64 flex-1"><span className="sr-only">Search this workspace</span><Search className="pointer-events-none absolute left-3 top-3.5 text-slate-400" size={18} /><input
          ...
          className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-10 pr-4 text-sm outline-none focus:border-emerald-700 focus:bg-white"
        /></label>
        {statuses.length > 1 && <label className="relative"><span className="sr-only">Filter by status</span><SlidersHorizontal className="pointer-events-none absolute left-3 top-3.5 text-slate-400" size={17} /><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-9 pr-8 text-sm sm:w-auto"><option value="all">All statuses</option>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label>}
        {canCreate && dedicatedEditor && <Link href={`/admin/manage/${collection}/new`} className="rounded-xl bg-emerald-900 px-5 py-3 text-center text-sm font-semibold text-white hover:bg-emerald-950">Create {collection.slice(0, -1)}</Link>}
        {fields.length > 0 && canCreate && !dedicatedEditor && <button onClick={() => { setCreating(true); setEditing(null); }} className="rounded-xl bg-emerald-900 px-5 py-3 text-sm font-semibold text-white hover:bg-emerald-950">Create record</button>}
      </div>
      ...
      {message && <p role="status" className="mt-4 rounded-xl bg-amber-100 px-4 py-3 text-sm text-amber-950">{message}</p>}
      <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white">
        {filtered.length === 0 ? (
          <div className="p-10 text-center text-slate-500">No records match this view.</div>
        ) : filtered.map((row) => (
          <article key={row.id} className="border-b border-slate-100 p-5 last:border-b-0">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="truncate font-semibold">{row.title}</h2><span className="rounded-md bg-slate-100 px-2 py-1 text-[0.66rem] font-bold uppercase tracking-wide text-slate-600">{row.status}</span></div><p className="mt-1 truncate text-sm text-slate-500">{row.subtitle || row.id}</p></div>
              ...
              {workflowStatuses && <button type="button" onClick={() => setExpanded((value) => value === row.id ? null : row.id)} className="inline-flex items-center gap-1 text-sm font-semibold text-slate-700">Details {expanded === row.id ? <ChevronUp size={15} /> : <ChevronDown size={15} />}</button>}
              {dedicatedEditor && <Link href={`/admin/manage/${collection}/${row.id}`} className="inline-flex items-center gap-1 text-sm font-semibold text-emerald-800">Edit <ExternalLink size={14} /></Link>}
              {fields.length > 0 && !dedicatedEditor && <button onClick={() => { setEditing(row); setCreating(false); }} className="text-sm font-semibold text-emerald-800">Edit</button>}
      ...
      {editorOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 p-4 sm:items-center" role="dialog" aria-modal="true" aria-label={editing ? "Edit record" : "Create record"}>
          <form onSubmit={submitEditor} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl sm:p-8">
            <div className="flex items-start justify-between gap-4">
              <div><p className="text-xs font-bold uppercase tracking-widest text-emerald-800">Structured editor</p><h2 className="mt-2 text-2xl font-semibold">{editing ? "Edit record" : "Create record"}</h2></div>
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              {fields.map((field) => {
                ...
                const className = "mt-2 w-full rounded-xl border border-slate-300 px-3 py-2.5 outline-none focus:border-emerald-700";
                return (
            ...
            <button disabled={busy} className="mt-7 rounded-xl bg-emerald-900 px-5 py-3 font-semibold text-white disabled:opacity-50">{busy ? "Saving…" : "Save record"}</button>
          </form>
        </div>
      )}
    </>
...
function RecordDetails({ row, collection, demo }: { row: AdminRow; collection: string; demo: boolean }) {
  ...
  return <div className="mt-5 grid gap-5 rounded-xl bg-slate-50 p-4 lg:grid-cols-[minmax(0,1fr)_320px]">
    ...
    {collection.includes("leads") || collection === "inquiries" ? <div><label className="text-xs font-bold uppercase tracking-wider text-slate-500">Internal notes<textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={4} className="mt-2 w-full rounded-xl border border-slate-300 bg-white p-3 text-sm" /></label><button type="button" onClick={saveNotes} className="mt-2 rounded-lg bg-slate-900 px-3 py-2 text-xs font-semibold text-white">Save notes</button>{message && <span className="ml-3 text-xs text-slate-600">{message}</span>}</div> : null}
  </div>;
}
