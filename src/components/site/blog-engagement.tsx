  ...
  return (
    <section className="blog-engagement" aria-labelledby="article-feedback-title">
      <div className="rating-panel">
        <div><p className="eyebrow">Reader feedback</p><h2 id="article-feedback-title">Was this article useful?</h2><p>{rating.count ? `${rating.average.toFixed(1)} out of 5 from ${rating.count} ${rating.count === 1 ? "rating" : "ratings"}.` : "Be the first reader to rate this article."}</p></div>
        <fieldset disabled={submittingRating}><legend>Rate from one to five stars</legend><div className="star-rating">{[1,2,3,4,5].map((value) => <button key={value} type="button" onClick={() => submitRating(value)} aria-label={`Rate ${value} out of 5`}><Star aria-hidden="true" size={22} /></button>)}</div></fieldset>
        {ratingStatus ? <p className={`form-notice form-notice--${ratingStatus.tone}`} role="status">{ratingStatus.message}</p> : null}
      </div>
      <div className="comments-panel">
        <div><p className="eyebrow">Community conversation</p><h2>Comments</h2><p>Comments are reviewed before they appear. Your email is used for moderation only and is never published.</p></div>
