...
The current stakeholder review environment is <https://church-govern-staging.vercel.app>. It uses `APP_ENV=staging`, `USE_MOCK_CONTENT=true`, and `USE_MOCK_OPERATIONS=false`: editorial content is provisional while Supabase operations and Resend notification requests are live. Server credentials are sensitive Vercel variables. It is shareable without Vercel SSO, remains `noindex`, and is not the production target. No Hostinger staging or production deployment has been verified as of 2026-08-22.
