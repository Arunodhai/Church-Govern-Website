...
Publish the accumulated local work. Everything from the chartreuse/mint palette through the route polish and the 2026-08-25 homepage motion pass is verified locally but unpushed, so <https://church-govern-staging.vercel.app> still shows the prior visual system. Before committing, add the fourteen installed agent-tool directories (`.agent`, `.agents`, `.claude`, `.codex`, `.cursor`, `.gemini`, `.hermes`, `.impeccable`, `.kiro`, `.omc`, `.opencode`, `.pi`, `.qoder`, `.vibe`) to `.gitignore` rather than committing roughly 40MB of vendored tooling, and leave `main img.png` out because it is a byte-identical duplicate of `public/images/church-govern-main-hero.png`. Then push, let Vercel build Preview, and repeat the desktop and 390-by-844 route/interaction checks plus a reduced-motion check against the stable URL.
